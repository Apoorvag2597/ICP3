...
function setPreviewColor(color) { //Calling a function
    $('.preview').css('background-color', color); //Displaying the color that is shown
...
$(document).on("click","#add-to-favorite",function(){
    var color=$('.preview').css('background-color'); //Adding the colors which are favorite and displaying iy
    addBox(color)
})
//3.Write events handlers such that whenever any item in the favorite colors is clicked or hovered, the color gets displayed in the preview div
    $(document).on("click mouseover",".item",function(){
        var color=$(this).css('background-color'); //When moved over the required color it is again displayed
        setPreviewColor(color);
    })
